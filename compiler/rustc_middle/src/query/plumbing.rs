            ...
            pub mod $name {
                ...
                impl<'tcx> crate::query::QueryHelper<'tcx, Key<'tcx>, Erased<Value<'tcx>>> for Helper {
                    ...
                    fn try_load_from_disk_fn(
                        ...
                        index: crate::dep_graph::DepNodeIndex,
                    ) -> Option<Erased<Value<'tcx>>> {
                        use crate::queries::$name::{ProvidedValue, provided_to_erased};
                        ...
                        #[cfg($separate_provide_extern)]
                        crate::query::AsLocalQueryKey::as_local_key(&_key)?;

                        let loaded_value: ProvidedValue<'tcx> =
                            $crate::query::plumbing::try_load_from_disk(tcx, prev_index, index)?;
                    ...
                    fn try_load_from_disk_fn(
                        ...
                        _index: crate::dep_graph::DepNodeIndex,
                    ) -> Option<Erased<Value<'tcx>>> {
                        None
                    }
                }
