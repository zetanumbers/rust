...
impl<D: Deps> DepGraph<D> {
        ...
        OP: FnOnce() -> R,
    {
        match self.data() {
            Some(data) => {
                let (result, index, cache) = data.with_anon_task_inner(cx, dep_kind, inline, op);
...
impl<D: Deps> DepGraphData<D> {
    ...
    pub(crate) fn with_anon_task_inner<Tcx: DepContext<Deps = D>, OP, R>(
        ...
        inline: bool,
        op: OP,
    ) -> (R, DepNodeIndex, Option<DepCache>)
    where
        OP: FnOnce() -> R,
    {
        ...
        let (dep_node_index, cache) = match task_deps.len() {
            ...
            1 => {
                ...
                (task_deps[0], Some(task_deps.cache(0)))
            }
            _ => {
                ...
                // Record only TraitSelect anon tasks as they are just closures inside some queries.
                ...
                let dep_index =
                    self.current.anon_node_to_index.get_or_insert_with(target_dep_node, || {
                        ...
                        self.current.alloc_new_node(
                            ...
                            task_deps,
                            Fingerprint::ZERO,
                            // we account for anon tasks with zero timeframe in the analysis
                            if !inline { timeframe } else { Duration::ZERO },
                        )
                    });
                (dep_index, (!inline && !collision).then_some(DepCache::Computed))
            }
        };
