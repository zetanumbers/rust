...
use rustc_middle::mir::{
    self, BasicBlockData, CallSource, ConstOperand, Local, LocalDecl, MirPass, Operand, Place,
    ProjectionElem, Rvalue, SourceInfo, Statement, StatementKind, SwitchTargets, Terminator,
    TerminatorKind, UnwindAction, START_BLOCK,
};
...
impl<'tcx> MirPass<'tcx> for AddAsyncDrop {
    fn run_pass(&self, tcx: TyCtxt<'tcx>, body: &mut rustc_middle::mir::Body<'tcx>) {
        // TODO: use MIR patches
        if let InstanceDef::Item(item) = body.source.instance
            && tcx.coroutine_is_async(item)
        {
            debug!("AddAsyncDrop({})", tcx.def_path_str(item));
            let param_env = tcx.param_env_reveal_all_normalized(item);

            // Get every block with drop inside of async context
            let blocks_count = body.basic_blocks.len();
            let mut blocks_to_add_async_drop = BitSet::new_empty(blocks_count);
            {
                let mut blocks_visited = BitSet::new_empty(blocks_count);
                // TODO: use WorkQueue?
                let mut blocks_to_reach = VecDeque::from([START_BLOCK]);
                while let Some(block) = blocks_to_reach.pop_back() {
                    if !blocks_visited.insert(block) {
                        continue;
                    }
                    if blocks_to_add_async_drop.contains(block) {
                        continue;
                    }
                    match &body.basic_blocks[block].terminator().kind {
                        TerminatorKind::Drop { replace, target, place, .. } => {
                            if place.ty(body, tcx).ty.is_async_drop(tcx, param_env) {
                                blocks_to_add_async_drop.insert(block);
                            }
                            blocks_to_reach.push_back(*target);
                        }
                        ...
                        TerminatorKind::SwitchInt { targets, .. } => {
                            blocks_to_reach.extend(targets.all_targets())
                        }
                        ...
                        TerminatorKind::FalseEdge { real_target, imaginary_target } => {
                            blocks_to_reach.extend([real_target, imaginary_target])
                        }
                    }
                }
            }
            ...
            let Discr { val: poll_ready_discr, ty: poll_discr_ty } = poll_enum
                .discriminant_for_variant(
                    tcx,
                    poll_enum.variant_index_with_id(
                        tcx.require_lang_item(LangItem::PollReady, item_span),
                    ),
                );
            let poll_pending_discr = poll_enum
                .discriminant_for_variant(
                    tcx,
                    poll_enum.variant_index_with_id(
                        tcx.require_lang_item(LangItem::PollPending, item_span),
                    ),
                )
                .val;
            ...
            for old_drop_block in blocks_to_add_async_drop.iter() {
                ...
                let drop_ref_place = Place {
                    local: body
                        .local_decls
                        .push(LocalDecl::with_source_info(drop_ref_ty, source_info)),
                    projection: List::empty(),
                };

                trace!("pin_new_unchecked_fn");
                ...
                let poll_drop_fn = Ty::new_fn_def(
                    ...
                    iter::once(GenericArg::from(drop_ty)).chain(
                        iter::repeat_with(|| {
                            Region::new_from_kind(tcx, RegionKind::ReErased).into()
                        })
                        .take(3),
                    ),
                );
                ...
                let drop_block_data = BasicBlockData::new(Some(drop_terminator));
                let drop_block = basic_blocks.push(drop_block_data);
                let drop_block_data = &basic_blocks[drop_block];
                ...
                // Defining blocks
                basic_blocks[old_drop_block].statements.extend(temporaries.iter().copied().map(
                    |local| Statement { source_info, kind: StatementKind::StorageLive(local) },
                ));
                ...
                basic_blocks[drop_block].statements.extend(temporaries.iter().copied().map(
                    |local| Statement { source_info, kind: StatementKind::StorageDead(local) },
                ));
            }
        }
    }
